...
function get_initial_plan(training_period_length, bidding_start, burned=0, two_price=false)
    ...
    @objective(initial_plan, Max,
        sum(
            ...
            E_DW[t] * 0.01
            -
            (lambda_UP[t+offset] + 0.01) * E_UP[t]
            for t in periods
        )
    )
...
bidding_start = length(lambda_F) - year
...
filename = "2020/optimal_everything_training_data"

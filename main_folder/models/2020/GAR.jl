...
function get_initial_plan(training_period_length, bidding_start)
    ...
    @variable(initial_plan, qF[1:(n_features+1), 1:3])
    @variable(initial_plan, qH[1:(n_features+1), 1:3])
    ...
    @objective(initial_plan, Max,
        sum(
            ...
            + lambda_H * hydrogen[t]
            + lambda_UP[t+offset] * E_DW[t]
            -
            lambda_DW[t+offset] * E_UP[t]
            for t in periods
        )
    )
...
for i in 1:12
    ...
    for v in 1:(validation_period+test_period)
        ...
        if (v % (720) == 0)
            ...
            filename = "2020/changing_qs_general/$(n_months)_month_training/general_pricedomain_RETRAINING_mo$(Int(round(v/720)))"
...
for i in 1:12
    ...
    for v in 1:(validation_period+test_period)
        ...
        if (v % (720) == 0)
            ...
            filename = "2020/changing_qs_general/$(n_months)_month_training/general_forecasted_model_pricedomain_RETRAINING_mo$(Int(round(v/720)))"
            easy_export(data, names, filename,)
...
for i in 1:12
    ...
    for v in 1:validation_period
        ...
        if (v % 720 == 0)
            ...
            filename = "2020/changing_qs_general/$(n_months)_month_training/general_rf_pricedomain_RETRAINING_mo$(Int(round(v/720)))"
            easy_export(data, names, filename,)

...
function get_SAA_plan(bidding_start)
   ...
   # @constraint(SAA, bidding[t in periods, s in scenarios], forward_bid[t] + hydrogen[t,s] == wind_real[t,s] )
    ...
    #Constraint so we dont up-regulate beyond what we have scheduled 
    @constraint(SAA,reg_hydrogen[t in periods, s in scenarios], EH_UP[t,s] <= hydrogen[t])
    @constraint(SAA,reg_hydrogen[t in periods, s in scenarios], E_UP[t,s] <= forward_bid[t])
    
    @constraint(SAA, elec_capacity_up[t in periods, s in scenarios], EH_UP[t,s] <= max_elec_capacity)
